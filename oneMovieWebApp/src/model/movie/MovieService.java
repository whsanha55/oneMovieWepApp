...
public class MovieService {
	...
	/*
	 * //영화를 삭제하다. public void deleteMovie(int movieNo) throws Exception {
	 * Connection conn = null; try { conn = DBConn.getConnection();
	 * 
	 * //tx.begin conn.setAutoCommit(false);
	 * 
	 * MovieDAO articleDao = MovieDAO.getInstance(); articleDao.removeMovie(conn,
	 * movieNo);
	 * 
	 * conn.commit();
	 * 
	 * } catch (Exception e) { conn.rollback(); throw e; } finally { if(conn !=
	 * null) conn.close(); } }
	 */
	// 영화를 삭제하다.
	...
	public void deleteMovieList(int movieNo) throws Exception {
		...
		try {
			...
			conn.setAutoCommit(false);
			/*
			 * //사진 삭제 PhotoDAO photoDao = PhotoDAO.getInstance();
			 * photoDao.removePhotoList(conn, movieNo);
			 * 
			 * //출연진 사진 삭제 ActorPhotoDAO actorPhotoDao = ActorPhotoDAO.getInstance();
	...
	public void updateMovie(MovieVO movie) throws Exception {
		...
		try {
			...
			// 출연진 수정
			/*
			 * ActorDAO actorDAO = ActorDAO.getInstance(); ActorVO actor =
			 * movie.getActors(); actor.setMovieNo(movie.getMovieNo());
	...
	// 영화 목록을 조회하다.
	// public List<MovieVO> retriveMovieList(String keyfield, String keyword, int
