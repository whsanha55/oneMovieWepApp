...
<script>
	...
	function jqueryPager(subOption) {
		...
		if (sPage > 1) {
			html += '<a onclick="goPage(' + 1 + ', ' + key
					+ ');">[ 처음 ]   </a>';
			html += '<a onclick="goPage(' + (sPage - pageBlock) + ', ' + key
					+ ');">[ 이전 ]   </a>';
		}
		...
		for (var i = sPage; i <= ePage; i++) {
			if (currentPage == i) {
				...
				html += '<a onclick="goPage(' + i + ', ' + key + ');">' + i
						+ '</a>';
			}
		}

		if (ePage < pageTotalCnt) {
			html += '<a onclick="goPage(' + (ePage + 1) + ', ' + key
					+ ');">   [ 다음 ]   </a>';
			html += '<a onclick="goPage(' + pageTotalCnt + ', ' + key
					+ ');">    [ 끝 ]</a>';
		}
						...
						$.ajax({
							...
							success : function(data,
								...
								for (var i = 0; i < data.length; i++) {
									...
									htmlStr += "<td class='title'>";
									htmlStr += "<a href='${pageContext.request.contextPath}/user/movie/detailMovie.do?movieNo=" + data[i].movieNo + " '>" + data[i].movieTitle + "</a></td>";
									htmlStr += "</tr>";
									htmlStr += "<tr>";
									...
									htmlStr += "</tr>";
									htmlStr += "<tr>";
									htmlStr += "<td class='last'>";
									htmlStr += "<a class='booking' href='${pageContext.request.contextPath}/memberBooking.do?movieNo=${pageScope.movie.movieNo}'>예매하기</a> &nbsp;&nbsp;";
									htmlStr += "<a class='theater' href='#'>상영시간표</a></td>";
									htmlStr += "</tr>";
